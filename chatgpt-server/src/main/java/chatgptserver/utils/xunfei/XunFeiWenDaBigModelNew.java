...
public class XunFeiWenDaBigModelNew extends WebSocketListener {
     ...
     * 存储从数据库中查询的历史聊天记录
     */
    private List<MessagesPO> historyChatList;

    // 构造函数
    public XunFeiWenDaBigModelNew(String userId, Boolean wsCloseFlag) {
        this.userId = userId;
        this.wsCloseFlag = wsCloseFlag;
    }

    public XunFeiWenDaBigModelNew(Long threadId, String content, List<MessagesPO> historyList, String userId, boolean wsCloseFlag) {
        this.threadId = threadId;
        this.content = content;
        this.historyChatList = historyList;
        this.userId = userId;
        this.wsCloseFlag = wsCloseFlag;
    }
    ...
    public static void main(String[] args) throws Exception {
      ...
      while (true){
          if(totalFlag){
              ...
              for (int i = 0; i < 1; i++) {
                  totalAnswer="";
                  WebSocket webSocket = client.newWebSocket(request, new XunFeiWenDaBigModelNew(i + "",
                          false));
              }
          }else{
              Thread.sleep(200);
    ...
    class MyThread extends Thread {
        ...
        public void run() {
            try {
                ...
                header.put("app_id",GPTConstants.GPT_KEY_MAP.get(GPTConstants.XF_XH_APPID_KEY));
                header.put("uid",UUID.randomUUID().toString().substring(0, 10));

                JSONObject parameter=new JSONObject(); // parameter参数
                JSONObject chat=new JSONObject();
                chat.put("domain", GPTConstants.XF_XH_QUESTION_DOMAIN);
...
//                }
                if (historyChatList.size() > 0) {
                    for (MessagesPO history : historyChatList) {
                        RoleContent replication = new RoleContent();
                        replication.setRole("assistant");
                        replication.setContent(history.getReplication());
                        text.add(0, JSON.toJSON(replication));

                        RoleContent question = new RoleContent();
                        question.setRole("user");
                        question.setContent(history.getReplication());
                        text.add(0, JSON.toJSON(question));
                    }
                }
...
//                sseEmitter.send(SseEmitter.event().comment(temp.content));
        ...
        if (myJsonParse.header.status == 2) {
            ...
            XunFeiUtils.questionFlagMap.put(threadId, true);
        }
    }
